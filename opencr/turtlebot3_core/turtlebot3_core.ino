...
void setup()
{
  ...
  nh.subscribe(start_servo_sub);
  nh.subscribe(left_right_sub);
  nh.subscribe(bti_sub);
  ...
  leftRightServo.attach(CAMERA_LEFTRIGHT_PIN);
  btiServo.attach(BTI_PIN);
...
/*******************************************************************************
* Callback function for pan-tilt servo motion
*******************************************************************************/
void startServoPositionCallback(const std_msgs::Bool& start_servo_msg){
  if(start_servo_msg.data){
    left_right_angle = 90;
    angle_msg.data = left_right_angle;
...
void leftRightCallback(const std_msgs::Bool& left_right_msg){
  //left
  if(left_right_msg.data){
    left_right_angle += 15;
    leftRightServo.write(left_right_angle);
    angle_msg.data = left_right_angle;
    angle_pub.publish(&angle_msg);
   //right
  }else if(!left_right_msg.data){
    left_right_angle -= 15;
    leftRightServo.write(left_right_angle);
    angle_msg.data = left_right_angle;
    angle_pub.publish(&angle_msg);
